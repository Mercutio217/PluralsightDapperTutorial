...
namespace MicroOrmDemo.DataLayer.Tests
{
    ...
    public class ContactRepoTests
    {
        ...
        static ResetScripts ResetScripts =
            new ResetScripts();

        [ClassInitialize]
        public void SetUp()
        {
            ResetScripts.CreateTables();
            ResetScripts.PopulateTables();
        }
        [ClassCleanup]
        public void TearDown()
        {
            ResetScripts.DropTables();
        ...
        {
           ResetScripts.CreateTables();
            var contacts = ResetScripts.CheckIfContactsExists();
            var addresses = ResetScripts.CheckIfAddressesExist();
            var states = ResetScripts.CheckIfStatesExist();

            Assert.AreEqual(true, contacts);
            Assert.AreEqual(true, addresses);
            Assert.AreEqual(true, states);
        }
        [TestMethod]
        public void TestPopulateTables()
        {
            ResetScripts.PopulateTables();
            var contacts = ResetScripts.CountRecords("Contacts");
            Assert.AreEqual(6, contacts);
        }


        [TestMethod]
        public void TestIfResetScriptReturnsRightValue()
        {
            var contacts = ResetScripts.CheckIfContactsExists();
            var addresses = ResetScripts.CheckIfAddressesExist();
            var states = ResetScripts.CheckIfStatesExist();
        ...
        [TestMethod]
        public void GetAllShouldReturn6Results()
        {
        ...
        public void Delete_should_remove_entity()
        {
            ...
            // create a new repoordesitory for verification purposes
            IContactRepository repository2 = CreateRepository();
